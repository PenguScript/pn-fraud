...
RegisterNetEvent('pn-fraud:GiveMoneyFromChecks', function(args)
    ...
    if ctype or amnt ~= nil or 0 then
        if ctype == "fraud" then
            local v1 = math.random(Config.FraudMoneyRewardValue.min,Config.FraudMoneyRewardValue.max)
            local value = v1*amnt
        ...
        elseif ctype == "real" then
            local v1 = math.random(Config.RealCheckMoneyRewardValue.min,Config.RealCheckMoneyRewardValue.max)
            local value = v1*amnt
